...
echo "Creating"
location=$(cat vm)
rs=$(cat rs) && az vm create --resource-group $rs --name win33 --image MicrosoftWindowsDesktop:windows-ent-cpc:win33-21h2-ent-cpc-m365:22000.376.2112141747 --public-ip-sku Standard --size Standard_DS2_v2 --location $location --admin-username azureuser --admin-password WindowsPassword@001 --out table
...
az vm open-port --resource-group $rs --name win33 --port '*' --output none

echo " Done! "
IP=$(az vm show -d -g $rs -n win33 --query publicIps -o tsv)
echo "Public IP: $IP"
